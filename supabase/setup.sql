-- ══════════════════════════════════════════════════════════════════════
-- OS MANAGER — SETUP COMPLETO DO BANCO v2
...
CREATE INDEX IF NOT EXISTS idx_orders_public_token    ON public.service_orders(public_token);
...
BEGIN
  IF NEW.quote_number IS NULL OR NEW.quote_number = '' THEN
    SELECT COUNT(*) + 1 INTO seq FROM public.quotes WHERE user_id = NEW.user_id;
...
BEGIN
  ...
  SET
    parts_total = (SELECT COALESCE(SUM(total_price), 0) FROM public.quote_items WHERE quote_id = COALESCE(NEW.quote_id, OLD.quote_id)),
    total_value = labor_value + (SELECT COALESCE(SUM(total_price), 0) FROM public.quote_items WHERE quote_id = COALESCE(NEW.quote_id, OLD.quote_id)),
    updated_at  = NOW()
...
-- Sincroniza pagamentos com a OS
...
DECLARE
  ...
  v_date   DATE;
BEGIN
  SELECT service_value INTO v_total
  FROM public.service_orders WHERE id = COALESCE(NEW.order_id, OLD.order_id);

  SELECT COALESCE(SUM(amount), 0) INTO v_paid
  FROM public.order_payments WHERE order_id = COALESCE(NEW.order_id, OLD.order_id);
  ...
  SELECT method, paid_at INTO v_method, v_date
...
CREATE OR REPLACE TRIGGER trg_sync_payment
...
BEGIN
  ...
  INSERT INTO public.order_items (order_id, part_id, quantity, unit_price, total_price)
...
CREATE POLICY "notes_select" ON public.order_notes FOR SELECT USING (
  EXISTS (SELECT 1 FROM public.service_orders WHERE id = order_notes.order_id AND user_id = auth.uid())
);
...
-- order_payments
CREATE POLICY "order_payments_select" ON public.order_payments FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "order_payments_insert" ON public.order_payments FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "order_payments_delete" ON public.order_payments FOR DELETE USING (auth.uid() = user_id);
...
-- company_settings — leitura pública para tela de login
CREATE POLICY "settings_select" ON public.company_settings FOR SELECT USING (true);
...
--   VITE_COMPANY_USER_ID  (user_id do cliente — para logo na tela de login)
-- ══════════════════════════════════════════════════════════════════════